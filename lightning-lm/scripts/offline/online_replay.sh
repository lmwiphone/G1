...
/opt/G1/lighting_ws/install/robot_bringup/lib/robot_bringup/robot_pose_pub_node > "$V/pose_pub.out" 2>&1 & POSE=$!
sleep 5
S=$(date +%s)
ros2 bag play "$BAG" --topics /livox/lidar /livox/imu /tf_static --qos-profile-overrides-path "$V/qos.yaml" > "$V/play.out" 2>&1
